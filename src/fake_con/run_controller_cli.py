...
from .joycontrol import debug, utils
...
async def test_controller_buttons(controller_state: ControllerState):
    ...
    await ainput(
        prompt="Make sure the Switch is in the Home menu and press <enter> to continue."
    )
...
async def update_stick(controller_state, analogs, joystick):
    for key in analogs:
        if key[-6:] == "analog":  # analog sticks
            ...
            vals = {}
            vals["h"] = abs(
                int(((val_h + 1) / 2) * 4096) - 1
            )  # converts to the range of [0, 4096)
            vals["v"] = abs(
                int(((-1 * val_v + 1) / 2) * 4096) - 1
...
async def relais(controller_state):
    ...
    buttons = {val: key for key, val in buttons.items()}
...
async def _main(args):
    ...
    with utils.get_output(path=args.log, default=None) as capture_file:
        ...
        cli.add_command(
            "amiibo",
            ControllerCLI.deprecated('Command was removed - use "nfc" instead!'),
        )
...
def main():
    ...
    parser.add_argument(
        "-d", "--device_id", help="not fully working yet, the BT-adapter to use"
    )
