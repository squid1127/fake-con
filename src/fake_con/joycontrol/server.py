...
async def create_hid_server(
    ...
    if reconnect_bt_addr is None:
        if interactive:
            ...
            if b:
                logger.warning(
                    f"Attempting initial pairing, but switches are paired: {b}"
                )
        ...
        try:
            ...
            logger.warning(
                'Fallback: Restarting bluetooth due to incompatibilities with the bluez "input" plugin. '
    ...
    asyncio.ensure_future(_send_empty_input_reports(transport))
